...
CI measures independently on Ubuntu 24.04 with the pinned Playwright Chromium, tests anchor navigation and remembered chunk geometry, and compares measured chunk heights with the committed artifact (1 CSS pixel tolerance). Its measured JSON is available as the `render-calibration-*` artifact when a geometry update is needed. A new release measures its own candidate after DOI/version promotion and commits that calibration before freezing the source SHA; resumed and current releases validate their already-committed calibration. CI artifacts are never substituted into a frozen release. Run `npm run test:render-calibration` for invalid/stale-data regression tests and `npm run test:render-navigation` after a full build for browser navigation tests.
...
The canonical Dataset is `https://www.ghezelbaash.ir/graph.jsonld#dataset`. GitHub is its version-controlled source, Zenodo is its immutable DOI distribution, Hugging Face `main` is its current AI/retrieval distribution, and versioned Hugging Face tags preserve frozen release snapshots. Release promotion updates the release record, graph, package metadata and citation metadata as one transaction; the public evidence snapshot is derived from the evidence registry during generation, and external publication remains an explicit release operation. The `External authority release contract` workflow accepts a version through `workflow_dispatch` and pins the selected branch to the dispatch commit. New candidates must descend from both the unchanged canonical `main` and that selected source; current-release reconciliation is limited to `main`. The reserved Zenodo DOI is embedded before the candidate is built and published, and the latest public Zenodo version must match the registered publication history even when an earlier publication failed to reach GitHub or Hugging Face.

The current release is read from canonical `main`, so an unpublished candidate can be corrected and resumed with its existing DOI. Before immutable publication, the workflow applies and reads back the native Cloudflare zone settings, including disabled Browser Integrity Check for public machine access, then checks that a standard Python client can read the canonical graph directly. Zenodo file staging reports each operation and recovers from transient failures within a bounded retry window by reading the draft and file inventory again. A file is reused only when its downloaded bytes match; all staged bytes are checked again before publication.
