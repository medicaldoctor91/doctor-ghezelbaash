...
The preserved source release is **${release.release}**, identified by DOI **${zenodo.versionDoi}**. The [v${release.release} snapshot](${release.dataset.huggingFace.dataset}/tree/v${release.release}) pins the released Hugging Face representation. Typed Parquet files and this card are access derivatives, not independent evidence. Access packaging revision: **${packaging.packagingRevision}**. Pin a full Hugging Face commit when reproducing access packaging; later packaging corrections on \`main\` must not alter the DOI-preserved source resources. [dist-sha256.json](${release.dataset.huggingFace.dataset}/resolve/main/dist-sha256.json) covers the declared file inventory, and the packaging manifest binds each derivative to its source hash.
