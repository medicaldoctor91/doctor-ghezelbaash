...
const budgetProbe=await request('/','Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)');const budgetBodyBytes=Buffer.byteLength(budgetProbe.text),budgetHeaderBytes=[...budgetProbe.r.headers].reduce((n,[k,v])=>n+Buffer.byteLength(`${k}: ${v}\r\n`),0);if(budgetBodyBytes>=inv.maxHtmlBytes||budgetBodyBytes+budgetHeaderBytes+inv.googlebotSafetyMarginBytes>inv.googlebotFetchBudgetBytes)fail(`Production Googlebot response budget unsafe body=${budgetBodyBytes} headers=${budgetHeaderBytes}`);const cssHref=(budgetProbe.text.match(/<link\b[^>]*(?:rel=["']stylesheet["'][^>]*href|href)=["'](\/assets\/site\.[0-9a-f]{12}\.css)["'][^>]*>/i)||[])[1]||(budgetProbe.text.match(/<link\b[^>]*href=["'](\/assets\/site\.[0-9a-f]{12}\.css)["'][^>]*rel=["']stylesheet["'][^>]*>/i)||[])[1];if(!cssHref)fail('Production fingerprint stylesheet link missing');const cssProbe=await request(cssHref,'Googlebot');if(cssProbe.r.status!==200||!/max-age=31536000/i.test(cssProbe.r.headers.get('cache-control')||''))fail(`Production fingerprint stylesheet/cache drift ${cssProbe.r.status} ${cssProbe.r.headers.get('cache-control')||''}`);if((budgetProbe.text.match(/<section\b[^>]*class=["'][^"']*direct-answer-capsules[^"']*["'][^>]*>[\s\S]*?<h3\b/gi)||[]).length<1||(budgetProbe.text.match(/<article\b[^>]*data-answer-id=["'][^"']+["'][^>]*>[\s\S]*?<h3>/gi)||[]).length!==inv.directAnswerCapsuleCount)fail('Production Direct Answer H3 hierarchy drift');
