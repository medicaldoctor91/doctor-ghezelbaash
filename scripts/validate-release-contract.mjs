...
const exactKeys = (value, expected, label) => {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(wanted))
    fail(`${label} schema mismatch: ${actual.join(",")}`);
};
...
if (
  pkg.version !== R ||
  lock.version !== R ||
  lock.packages?.[""]?.version !== R
)
  fail("Package release identity drift");
if (!validDate(release.dateModified) || !validDate(release.medicalReviewedAt))
  fail("Release/review dates must be ISO dates");
...
const redirectsSha256 = createHash("sha256")
  .update(redirectsBytes)
  .digest("hex");
...
for (const entry of Z.releaseHistory) {
  exactKeys(
    entry,
    ["publicationDate", "recordId", "release", "versionDoi"],
    "releaseHistory entry",
  );
  if (
    !validSemver(entry.release) ||
    !validDate(entry.publicationDate) ||
    !validDoi(entry.versionDoi) ||
    !validRecord(entry.recordId)
  )
    fail(`Malformed releaseHistory entry: ${entry.release}`);
  if (
    releases.has(entry.release) ||
    records.has(String(entry.recordId)) ||
    dois.has(entry.versionDoi)
  )
    fail(`Duplicate releaseHistory identity: ${entry.release}`);
  if (previousDate && entry.publicationDate < previousDate)
    fail("releaseHistory must be publication-date ordered");
  previousDate = entry.publicationDate;
  releases.add(entry.release);
  records.add(String(entry.recordId));
  dois.add(entry.versionDoi);
}
...
for (const token of [
  `version: ${R}`,
  `date-released: ${release.dateModified}`,
  `doi: ${Z.versionDoi}`,
])
  if (!citation.includes(token)) fail(`CITATION release drift: ${token}`);

const documentHead = await readFile(
  path.join(root, "src/components/DocumentHead.astro"),
  "utf8",
);
...
if (
  ...
  fail("Astro Head current-release DOI relation drift");
...
for (const removedId of [
  ...
  "doctor-ghezelbaash-structured-data-project",
  "doctor-ghezelbaash-structured-data-section",
  "data-catalog",
])
  if (pageSource.includes(`id="${removedId}"`))
    fail(
      `Machine Dataset landing block leaked into visible content: ${removedId}`,
    );
...
if (graphClosure.duplicateIds.length)
  fail(
    `Duplicate graph IDs: ${graphClosure.duplicateIds.map((item) => item.id).join(", ")}`,
  );
if (graphClosure.danglingSameSiteCount)
  fail(
    `Dangling same-site graph IDs: ${graphClosure.danglingSameSiteIds.join(", ")}`,
  );
...
if (
  ...
  fail("Dataset creator/publisher must resolve to the physician");
...
if (
  ...
  fail("Dataset about topology is incomplete");
...
const conditionIds = [
  ...
  "hyperpigmentation",
  "melasma",
].map((slug) => `${release.canonicalUrl}#biomedical-concept-${slug}`);
for (const conditionId of conditionIds) {
  const condition = byId.get(conditionId);
  if (!arr(condition?.["@type"]).includes("MedicalCondition"))
    fail(`MedicalCondition semantics missing: ${conditionId}`);
  const treatments = arr(condition.possibleTreatment).map(id).filter(Boolean);
  if (!treatments.length)
    fail(`MedicalCondition has no possibleTreatment: ${conditionId}`);
  for (const treatmentId of treatments)
    if (!arr(byId.get(treatmentId)?.["@type"]).includes("MedicalTherapy"))
      fail(
...
if (releaseBoundMismatches.length)
  fail(
    `Current release-bound graph metadata drift: ${releaseBoundMismatches.map((item) => item.id).join(", ")}`,
  );
...
if (catalog?.url !== `${release.canonicalUrl}dcat.ttl`)
  fail(
...
if (
  ...
  fail("Canonical JSON-LD must remain a Dataset DataDownload distribution");
if (
  JSON.stringify(github?.["@type"]) !== JSON.stringify("SoftwareSourceCode") ||
  ...
  id(github?.isPartOf) !== project?.["@id"] ||
  datasetDistributions.has(github?.["@id"]) ||
  !datasetSources.has(github?.["@id"])
)
  fail("SoftwareSourceCode source-role semantics drift");
for (const platformDistribution of [hf, zenodo])
  if (Object.hasOwn(platformDistribution || {}, "contentUrl"))
    fail(
      `Landing page must not be asserted as direct contentUrl: ${platformDistribution?.["@id"]}`,
    );
if (
  release.dataset.github?.role !== "source" ||
  release.dataset.github?.repository !== github?.codeRepository ||
  github?.version !== R
)
  fail("GitHub source-role projection drift");
if (
  release.dataset.huggingFace?.role !== "ai-distribution" ||
  release.dataset.huggingFace?.dataset !== hf?.url ||
  hf?.version !== R ||
  !String(hf?.description || "")
    .toLowerCase()
    .includes("ai") ||
  !String(hf?.description || "")
    .toLowerCase()
  ...
  fail("Hugging Face AI/retrieval distribution-role projection drift");
if (
  zenodo?.version !== R ||
  zenodo?.sameAs !== `https://zenodo.org/records/${Z.recordId}` ||
  !String(zenodo?.identifier || "").includes(Z.versionDoi) ||
  !String(zenodo?.url || "").includes(Z.versionDoi)
)
  fail("Zenodo preservation projection drift");

for (const entry of Z.releaseHistory) {
  const releaseNode = byId.get(
    releaseHistoryNodeId(release.canonicalUrl, entry.release),
  );
  if (
    !releaseNode ||
    !nodeTypes(releaseNode).has("Dataset") ||
    releaseNode.additionalType ||
    releaseNode.version !== entry.release ||
    releaseNode.datePublished !== entry.publicationDate ||
    !JSON.stringify(releaseNode).includes(entry.versionDoi)
  )
...
const machineResourceRegistry = await readJson(
...
for (const task of ["question-answering", "text-retrieval", "text-generation"])
  if (!hfPolicy.taskCategories?.includes(task))
    fail(`HF task contract missing: ${task}`);
for (const language of ["fa", "en", "ar", "ckb"])
  if (!retrievalPolicy.languages?.includes(language))
    fail(`Retrieval language contract missing: ${language}`);
for (const target of ["website", "huggingFace", "zenodo"])
  if (
    !machineResourceRegistry.resources.some((resource) =>
...
const resourcePaths = (target) =>
  machineResourceRegistry.resources
    ...
    .sort();
if (
  JSON.stringify(resourcePaths("huggingFace")) !==
  JSON.stringify(resourcePaths("zenodo"))
)
  fail("Immutable external resource inventories must match");
