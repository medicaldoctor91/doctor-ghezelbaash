...
for(const token of ['task_categories:','size_categories:','release.primaryEntity.wikidata','release.primaryEntity.googleKnowledgeGraphId','release.dataset.creatorOrcid','Personal Hugging Face identity'])if(!hfPrep.includes(token))fail(`Hugging Face physician-first discovery metadata missing ${token}`);
const mainHead=await readFile('src/data/templates/main-head.html','utf8');
