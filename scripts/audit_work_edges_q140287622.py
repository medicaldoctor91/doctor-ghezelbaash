...
s=requests.Session();s.headers.update({'User-Agent':'Q140287622-WorkEdgeAudit/1.0 (https://www.ghezelbaash.ir/)','Cache-Control':'no-cache'})
def get(**p):
...
if targets:
 ...
 for q,e in d.get('entities',{}).items():
  ...
  def cvs(p):return [{'claim_id':c.get('id'),'value':val(c.get('mainsnak')),'refs':len(c.get('references',[])or[]),'rank':c.get('rank')}for c in e.get('claims',{}).get(p,[])]
  entities[q]={'label':label,'lastrevid':e.get('lastrevid'),'modified':e.get('modified'),'P31':cvs('P31'),'P50_author':cvs('P50'),'P2093_author_name_string':cvs('P2093'),'P356_DOI':cvs('P356'),'P577_publication_date':cvs('P577'),'P1433_published_in':cvs('P1433'),'sitelinks':{k:v.get('title')for k,v in e.get('sitelinks',{}).items()}}
# mark symmetry
for edge in edges:
