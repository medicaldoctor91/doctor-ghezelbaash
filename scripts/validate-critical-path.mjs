...
const heroEarlyHint='Link: </media/images/physician/saeed-ghezelbash-portrait-960.497fc78613ac.avif>; rel=preload; as=image; type="image/avif"; media="screen and (max-width: 540px) and (min-resolution: 1.25dppx) and (max-resolution: 2.5dppx)"; fetchpriority=high';

const [release,documentHeadProfile,documentHead,baseLayout,indexSource,headersTemplate]=await Promise.all([
  readFile(path.join(root,'src/data/release.json'),'utf8').then(JSON.parse),
  readFile(path.join(root,'src/data/document-head.json'),'utf8').then(JSON.parse),
...
assert(documentHead.includes("document-head.json")&&documentHead.includes("release.json")&&documentHead.includes('HERO_PRELOAD_SRCSET')&&documentHead.includes("stage==='critical'")&&documentHead.includes("stage==='discovery'"),'Structured DocumentHead contract missing');
assert(!documentHead.includes('href={HERO_PRELOAD_HREF}'),'Responsive Hero preload must not trigger the fallback candidate in browsers that select from imagesrcset');
assert(headersTemplate.split(/\r?\n/).filter(line=>line.trim()===heroEarlyHint).length===1,'Mobile Hero Early Hint header drift');
assert(documentHead.includes('release.canonicalUrl'),'DocumentHead canonical URL binding missing');
assert(baseLayout.includes("release.json")&&baseLayout.includes('frontmatter.title')&&baseLayout.includes('frontmatter.description'),'BaseLayout canonical content metadata authority missing');
...
for(const field of ['title','description','robots','lang','dir','canonicalUrl'])assert(!Object.hasOwn(documentHeadProfile,field),`Document Head metadata may not own page field: ${field}`);
...
if(distArg){
  ...
  const [html,headers]=await Promise.all([readFile(path.join(dist,'index.html'),'utf8'),readFile(path.join(dist,'_headers'),'utf8')]);
  ...
  assert(headers.split(/\r?\n/).filter(line=>line.trim()===heroEarlyHint).length===1,'DIST mobile Hero Early Hint header drift');
  ...
  assert(html.includes(documentHeadProfile.openGraph.image)&&html.includes(documentHeadProfile.openGraph.imageAlt),'DIST social presentation profile drift');
  assert(!/static\.cloudflareinsights\.com/i.test(html),'Cloudflare Insights unexpectedly entered static DIST');
}

console.log(JSON.stringify({stage:'CRITICAL_PATH',headAuthority:'astro-native-structured-three-lane',contentMetadataAuthority:'markdown-frontmatter',canonicalUrlAuthority:'release.json',presentationAuthority:'document-head.json',generatedContentAuthority:'.generated/content/home.md',discoveryRendering:'astro-native',distValidated:Boolean(distArg),integrity:'PASS'},null,2));
