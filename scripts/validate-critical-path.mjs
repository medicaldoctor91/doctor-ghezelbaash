...
const [release,headProfile,documentHead,baseLayout,indexSource,headersTemplate]=await Promise.all([
...
assert(documentHead.includes("head-profile.json")&&documentHead.includes("release.json")&&documentHead.includes('HERO_PRELOAD_SRCSET')&&documentHead.includes("stage==='critical'")&&documentHead.includes("stage==='discovery'"),'Structured DocumentHead contract missing');
