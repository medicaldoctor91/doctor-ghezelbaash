import { readFileSync, writeFileSync } from 'node:fs';
import { join } from 'node:path';
import { homepageSections } from '../src/domain/homepage-sections.mjs';
import { homepageArticleSubsections } from '../src/domain/homepage-article-registry.mjs';
import { homepageSubsectionSummaries } from '../src/domain/homepage-subsection-summaries.mjs';

const homepagePath = join(process.cwd(), 'dist', 'index.html');
const source = readFileSync(homepagePath, 'utf8');
const withoutLegacyIds = source.replace(/\s+id="clinical-decision-model-[^"]+"/gu, '');
let cleaned = withoutLegacyIds;
let registryTitlesApplied = 0;
let subsectionSummariesApplied = 0;
let descriptiveCrossLinksApplied = 0;

for (const section of homepageSections) {
  const headingPattern = new RegExp(`(<h2\\b[^>]*id="${section.id}-title"[^>]*>)[\\s\\S]*?(<\\/h2>)`, 'u');
  if (!headingPattern.test(cleaned)) throw new Error(`Canonical Homepage H2 is absent from final HTML: ${section.id}`);
  cleaned = cleaned.replace(headingPattern, `$1${section.title}$2`);
  registryTitlesApplied += 1;
}

for (const subsection of homepageArticleSubsections) {
  const rawCrossLink = new RegExp(`(<a\\b[^>]*href="#${subsection.id}"[^>]*>)${subsection.id}(<\\/a>)`, 'gu');
  const matches = cleaned.match(rawCrossLink)?.length ?? 0;
  if (matches) {
    cleaned = cleaned.replace(rawCrossLink, `$1${subsection.title}$2`);
    descriptiveCrossLinksApplied += matches;
  }
}

for (const [subsectionId, summary] of Object.entries(homepageSubsectionSummaries)) {
  const headingPattern = new RegExp(`(<h3\\b[^>]*id="${subsectionId}-title"[^>]*>[\\s\\S]*?<\\/h3>)`, 'u');
  if (!headingPattern.test(cleaned)) throw new Error(`Summary destination is absent from final HTML: ${subsectionId}`);
  cleaned = cleaned.replace(headingPattern, `$1<p class="subsection-direct-answer"><strong>پاسخ مستقیم:</strong> ${summary}</p>`);
  subsectionSummariesApplied += 1;
}

writeFileSync(homepagePath, cleaned);

console.log(JSON.stringify({
  status: 'pass',
  removedLegacyNumericIds: (source.match(/\s+id="clinical-decision-model-[^"]+"/gu) ?? []).length,
  registryTitlesApplied,
  descriptiveCrossLinksApplied,
  subsectionSummariesApplied,
}, null, 2));
