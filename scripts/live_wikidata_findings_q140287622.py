...
s=requests.Session(); s.headers.update({'User-Agent':'Q140287622-Findings/1.0 (https://www.ghezelbaash.ir/)','Cache-Control':'no-cache'})
def get(**p):
...
# statement/ref hygiene
no_refs=[]; multi_url=[]; duplicates=[]; allrows=[]
for p,cs in ent.get('claims',{}).items():
    seen={}
    for c in cs:
        x=val(c.get('mainsnak'))
        quals={qp:[val(z) for z in qsn] for qp,qsn in c.get('qualifiers',{}).items()}
        refs=c.get('references',[]) or []
        hosts=[]; groups=[]
        for ref in refs:
            urls=[val(z) for z in ref.get('snaks',{}).get('P854',[]) if val(z)]
            groups.append(urls); hosts.extend([urlparse(u).netloc for u in urls if isinstance(u,str)])
...
out={'audit_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'lastrevid':ent.get('lastrevid'),'modified':ent.get('modified'),'sitelinks':{k:v.get('title') for k,v in ent.get('sitelinks',{}).items()},'constraint_violations':viol,'exact_duplicates':duplicates,'no_reference_statements':no_refs,'multi_url_reference_groups':multi_url,'all_statements':allrows}
