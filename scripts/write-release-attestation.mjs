...
if (!/^[0-9a-f]{40}$/.test(headCommit))
  throw new Error("HEAD is not an exact 40-character source commit");
...
if (requestedCommit && requestedCommit !== headCommit)
  throw new Error(
    `Release source commit mismatch: requested ${requestedCommit}, HEAD ${headCommit}`,
  );
...
const attestation = {
  ...
  queryMatrixSha256: await sha(".generated/projections/query-matrix.jsonl"),
  currentReleaseMatrixSha256: await sha(
...
await writeFile(
  ".release/runtime/release-attestation.json",
