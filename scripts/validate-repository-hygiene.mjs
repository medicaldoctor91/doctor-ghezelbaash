...
const generatedPrefixes=['dist/','release/','node_modules/','.python-deps/','.release/runtime/','.release/huggingface/'];
const internalNoteNames=new Set(['notes.md','dev-notes.md','internal-notes.md']);
const temporaryPrefixes=['audit-','backup-','draft-','scratch-','temp-','tmp-'];
const temporarySuffixes=['.bak','.old','.orig','.rej','.tmp','~'];
...
const nonMainHeadRef=/refs\/heads\/(?!main(?:\b|\/))[A-Za-z0-9._\/-]+/g;
const workflowBranchList=/^\s*branches:\s*\[([^\]]+)\]\s*$/gm;

for(const name of tracked){
  const normalized=name.replaceAll('\\','/');
  if(generatedPrefixes.some(prefix=>normalized.startsWith(prefix)))throw new Error(`Generated/runtime material must not be tracked: ${normalized}`);
  const base=path.posix.basename(normalized).toLowerCase();
  if(internalNoteNames.has(base))throw new Error(`Internal development note must not be tracked: ${normalized}`);
  if(temporaryPrefixes.some(prefix=>base.startsWith(prefix)))throw new Error(`Temporary working file must not be tracked: ${normalized}`);
  if(temporarySuffixes.some(suffix=>base.endsWith(suffix)))throw new Error(`Backup/editor residue must not be tracked: ${normalized}`);
}
...
for(const name of tracked){
  ...
  const refMatch=content.match(nonMainHeadRef);
  if(refMatch)throw new Error(`Non-main branch dependency in ${name}: ${refMatch[0]}`);
  nonMainHeadRef.lastIndex=0;
  if(name.startsWith('.github/workflows/')){
    for(const match of content.matchAll(workflowBranchList)){
      const branches=match[1].split(',').map(value=>value.trim().replace(/^['"]|['"]$/g,'')).filter(Boolean);
      if(branches.some(branch=>branch!=='main'))throw new Error(`Workflow branch dependency must be main-only in ${name}: ${branches.join(', ')}`);
    }
  }
