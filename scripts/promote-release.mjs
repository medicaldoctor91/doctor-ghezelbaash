...
for(const h of z.releaseHistory){const rid=releaseHistoryNodeId(release.canonicalUrl,h.release);let n=byId.get(rid);if(!n){n={'@id':rid};nodes.push(n);byId.set(rid,n)}Object.assign(n,{'@type':'Dataset',name:`Dr. Saeed Ghezelbash Public Knowledge Graph — Version ${h.release}`,version:h.release,datePublished:h.publicationDate,identifier:[{'@type':'PropertyValue',propertyID:'Zenodo Version DOI',value:h.versionDoi,url:`https://doi.org/${h.versionDoi}`},{'@type':'PropertyValue',propertyID:'Zenodo Record ID',value:String(h.recordId)}],isPartOf:{'@id':release.dataset.id},url:`https://doi.org/${h.versionDoi}`});delete n.additionalType}
