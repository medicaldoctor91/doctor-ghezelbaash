...
async function walk(directory, { bounded = false } = {}) {
  ...
  for (const entry of (await readdir(directory, { withFileTypes: true })).sort(
    ...
    if (entry.isDirectory())
      output.push(...(await walk(absolute, { bounded })));
...
async function inspectPhysicalInventory(stage) {
  ...
  for (const file of rasters) {
    ...
    if (!match)
      throw new Error(
        `${stage}: unfingerprinted raster outside compiler contract: ${relative}`,
      );
    const logical = relative.replace(fingerprintPattern, "$2");
    if (logicalToPhysical.has(logical))
      throw new Error(
        `${stage}: duplicate physical raster for logical asset ${logical}`,
      );
    const bytes = await readFile(file);
    if (sha256(bytes).slice(0, 12) !== match[1].toLowerCase())
      throw new Error(`${stage}: fingerprint mismatch ${relative}`);
  ...
  const unexpected = [...actualSet]
    .filter((logical) => !manifestSet.has(logical))
    .sort();
  if (missing.length || unexpected.length)
    throw new Error(
      `${stage}: media manifest boundary violation; missing=${missing.join(", ") || "none"}; unexpected=${unexpected.join(", ") || "none"}`,
    );
  if (rasters.length !== manifestSet.size)
    throw new Error(
      `${stage}: physical raster count ${rasters.length} does not match manifest ${manifestSet.size}`,
    );
...
async function captureSnapshot() {
  const transactionRoot = await mkdtemp(
    path.join(tmpdir(), "ghezel-media-transaction-"),
  );
  const mediaBackup = path.join(transactionRoot, "media");
  await cp(mediaRoot, mediaBackup, {
    recursive: true,
    preserveTimestamps: true,
  });
  const writableTextFiles = (await walk(root, { bounded: true }))
    .filter((file) => textPattern.test(file) && compilerWritableConsumer(file))
    .sort();
  const textSnapshot = await Promise.all(
    writableTextFiles.map(async (file) => ({
      file,
...
async function restoreSnapshot(snapshot) {
  ...
  try {
    ...
    await cp(snapshot.mediaBackup, mediaRoot, {
      recursive: true,
      preserveTimestamps: true,
    });
  } catch (error) {
    rollbackErrors.push({ surface: "public/media", message: error.message });
  }
  for (const entry of snapshot.textSnapshot) {
    try {
      await writeFile(entry.file, entry.bytes);
    } catch (error) {
      rollbackErrors.push({
        surface: projectRelative(entry.file),
...
if (preflightOnly) {
  console.log(
    JSON.stringify(
      {
        mediaEnrichmentBoundary: "MANIFEST_LOCKED",
        mode: "PREFLIGHT_ONLY",
        manifest: "src/data/media-dimensions.tsv",
        logicalRasterAssets: manifestSet.size,
        mutation: false,
        preflight: "PASS",
      },
      null,
...
try {
  const worker = spawnSync(
    process.execPath,
    [path.join(root, "scripts/enrich-image-metadata.mjs")],
    {
      cwd: root,
      env: process.env,
      stdio: "inherit",
  ...
  if (worker.status !== 0)
    throw new Error(
      `Media metadata worker failed with exit code ${worker.status}`,
    );
  ...
  try {
    await inspectPhysicalInventory("POST_ROLLBACK");
  } catch (verificationError) {
    rollbackErrors.push({
      surface: "rollback-verification",
...
console.log(
  JSON.stringify(
    {
      mediaEnrichmentBoundary: "MANIFEST_LOCKED",
      transaction: "MEDIA_ENRICHMENT_TRANSACTION",
      manifest: "src/data/media-dimensions.tsv",
      logicalRasterAssets: manifestSet.size,
      worker: "scripts/enrich-image-metadata.mjs",
      snapshotTraversal: "BOUNDED",
      preflight: "PASS",
      postflight: "PASS",
      rollback: "BYTE_SNAPSHOT",
      committed,
    },
    null,
    2,
