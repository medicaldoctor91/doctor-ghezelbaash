...
import {SITE_PRESENTATION_CONTRACT} from '../src/lib/site-presentation.mjs';
...
assert(HERO_SUBTITLE_PRESENTATION_CONTRACT.contextLabel==='پزشک زیبایی در کرمانشاه'&&HERO_SUBTITLE_PRESENTATION_CONTRACT.contextTreatment==='asymmetric-rail','Hero context rail contract drift');
assert(HERO_SUBTITLE_PRESENTATION_CONTRACT.visualSemanticSeparator===false&&HERO_SUBTITLE_PRESENTATION_CONTRACT.visualNameSuffix===':'&&!HERO_SUBTITLE_PRESENTATION_CONTRACT.identityCentered&&HERO_SUBTITLE_PRESENTATION_CONTRACT.identityAxis==='rtl-start','Hero identity presentation contract drift');
...
assert(SITE_PRESENTATION_CONTRACT.direction==='diagnostic-surrealism'&&!SITE_PRESENTATION_CONTRACT.decorativeDomAdded&&!SITE_PRESENTATION_CONTRACT.imageAssetsChanged&&!SITE_PRESENTATION_CONTRACT.imageGeometryChanged,'Site art direction violated source/image constraints');
...
assert(delivery.criticalCss.includes('.hero-title__descriptor{order:-1;display:flex;gap:.7rem;align-items:center')&&delivery.criticalCss.includes('.hero-title__descriptor::after{content:"";height:1px;flex:1;background:var(--line)}'),'Asymmetric context rail missing');
...
assert(delivery.criticalCss.includes('.hero-title__name{font-size:clamp(2.05rem,1.05em,3rem);font-weight:820;line-height:1.14}')&&delivery.criticalCss.includes('.hero-title__name::after{content:":"}'),'Physician identity hierarchy missing');
assert(delivery.criticalCss.includes('justify-self:start;max-width:30ch')&&delivery.criticalCss.includes('line-height:1.55;text-align:start;text-wrap:balance'),'RTL-start manifesto typography missing');
...
assert(delivery.criticalCss.includes('font-family:"Vazirmatn",system-ui,sans-serif'),'Hero lost explicit Persian typography authority');
...
console.log(JSON.stringify({stage:'DIAGNOSTIC_SURREALISM_2026',semanticH1:'UNCHANGED',contextLabel:'ASYMMETRIC_RAIL',visualSemicolon:'HIDDEN',identity:'RTL_START',manifesto:'RTL_START_INLINE_STOP',portraitFooter:'WHITE_EDITORIAL',mobileRhythm:'COMPACT',search:'COORDINATED',heroOrder:'UNCHANGED',imageAssets:'UNCHANGED',criticalBytes:Buffer.byteLength(delivery.criticalCss),criticalBudget:invariants.maxCriticalCssBytes,status:'PASS'},null,2));
