...
assert(HERO_SUBTITLE_PRESENTATION_CONTRACT.visualH1Order==='profession-location-first'&&HERO_SUBTITLE_PRESENTATION_CONTRACT.visualNameSuffix===':','Hero visual identity order contract drift');
...
assert(delivery.criticalCss.includes('.hero-title__descriptor{order:1;'),'Visual profession/location-first title order missing');
assert(delivery.criticalCss.includes('.hero-title__semantic-separator{order:2;'),'Visual title separator order missing');
assert(delivery.criticalCss.includes('.hero-title__name{order:3;flex-basis:100%;'),'Visual physician-name line missing');
assert(delivery.criticalCss.includes('.hero-title__name::after{content:":"}'),'Visual physician-name colon missing');
assert(delivery.criticalCss.includes('justify-self:center;max-width:32ch')&&delivery.criticalCss.includes('text-align:center;text-wrap:balance'),'Centered Hero manifesto typography missing');
...
assert(delivery.criticalCss.includes('.entity-hero .hero-title{margin-block-end:.72rem}')&&delivery.criticalCss.includes('.entity-hero .hero-subtitle{margin-block:.05rem .8rem}'),'Mobile masthead/manifesto spacing missing');
