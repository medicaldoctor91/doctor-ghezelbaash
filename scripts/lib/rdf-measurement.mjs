...
export const RDF_MEASUREMENT_CONTRACT = Object.freeze({
  ...
  canonicalSyntax:
    "canonical-n-quads-default-graph-as-ntriples-compatible-turtle",
});

export async function canonicalizeRdfDocument(document) {
  const raw = await jsonld.toRDF(document, {
  ...
  if (!Number.isSafeInteger(triples) || triples <= 0)
    throw new Error(`Invalid canonical RDF triple measurement: ${triples}`);
  return { text, triples };
}
