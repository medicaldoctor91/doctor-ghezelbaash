...
export async function compilePageAssets(context) {
  ...
  if (!assembled.names.length)
    throw new Error("Canonical modular content source is empty");
  ...
  if (
    !externalCss.includes("/*DIST_CHUNK_INTRINSIC_START*/") ||
    !externalCss.includes("/*DIST_CHUNK_INTRINSIC_END*/") ||
    !externalCss.includes(
      `/*DIST_CHUNK_CALIBRATION_SHA256:${calibration.sha256}*/`,
    )
  )
    throw new Error("External CSS lost assembled render calibration");

  await mkdir(generatedAssets, { recursive: true });
  await writeFile(path.join(generatedAssets, assetName), externalCss);
  return {
    home: assembled.content,
    externalCssAssetName: assetName,
    calibration,
  };
}
