...
export async function compileContactDiscovery(context) {
  ...
  const personPortrait = byId.get(
    `${release.canonicalUrl}#image-saeed-ghezelbash-portrait-master`,
  );
  const clinicImages = (
    Array.isArray(clinic?.image)
      ? clinic.image
      : [clinic?.image].filter(Boolean)
  ).map((value) => value?.["@id"]);
  const clinicPhoto = byId.get(
    clinicImages.find(
      (id) =>
        id &&
        id !== `${release.canonicalUrl}#image-doctor-ghezelbaash-clinic-logo`,
    ),
  );
  ...
  const doctorVcf = vCard(
    [
      ...
      personPortrait?.contentUrl
        ? `PHOTO;MEDIATYPE=image/jpeg:${personPortrait.contentUrl}`
        : "",
  ...
  const clinicVcf = vCard(
    [
      ...
      "FN:کلینیک زیبایی دکتر سعید قزلباش",
      "ORG:کلینیک زیبایی دکتر سعید قزلباش",
      `TEL;TYPE=work,voice:${clinic?.telephone || ""}`,
      `ADR;TYPE=work:;;${vEsc(addressNode?.streetAddress)};${vEsc(addressNode?.addressLocality)};${vEsc(addressNode?.addressRegion)};${vEsc(addressNode?.postalCode)};${vEsc(addressNode?.addressCountry)}`,
      `URL:${release.canonicalUrl}`,
      `SOURCE:${release.canonicalUrl}clinic.vcf`,
      clinicPhoto?.contentUrl
        ? `PHOTO;MEDIATYPE=image/webp:${clinicPhoto.contentUrl}`
        : "",
  ...
  const clinicImageUrls = (
    ...
    (value) =>
      typeof value === "string" && value.startsWith(release.canonicalUrl),
  );
  const imageLocs = [
    ...new Set([
      ...imageIds.map((id) => byId.get(id)?.contentUrl).filter(Boolean),
      ...clinicImageUrls,
    ]),
  ];
  ...
  for (const video of videos) {
    const thumb = valueText(
      video.thumbnailUrl || video.thumbnail?.contentUrl || video.thumbnail,
    );
