...
export function assertIdentityFingerprintSource(release) {
  ...
  const required = [
    ...
    ["wikidata", fingerprint.wikidata],
    ["googleKnowledgeGraphId", fingerprint.googleKnowledgeGraphId],
    ["irimc", fingerprint.irimc],
    ["orcid", fingerprint.orcid],
  ...
  for (const [label, value] of required)
    if (typeof value !== "string" || !value.trim())
      throw new Error(`Identity fingerprint source missing ${label}`);
  if (!fingerprint.verifiedWebIdentityMesh.length)
    throw new Error(
      "Identity fingerprint source missing verified web identity mesh",
    );
  return fingerprint;
}
