...
const LIVE_REPUTATION_SLOT =
  '<div class="hero-caption-reputation" id="google-maps-clinic-reputation-current" data-live-reputation-slot></div>';
const persianNumber = (value, digits = 0) =>
...
const bindLiveReputation = async (root, content, release) => {
  ...
  if (
    ...
    throw new Error("Invalid live reputation source for visible binding");
  const slotCount = String(content).split(LIVE_REPUTATION_SLOT).length - 1;
  if (slotCount !== 1)
    throw new Error(
      `Expected one exact visible reputation slot; found ${slotCount}`,
    );
  const replacement =
    '<div class="hero-caption-reputation" id="google-maps-clinic-reputation-current"><strong>' +
    persianNumber(rating, 1) +
    ' از ۵ در <span translate="no">Google Maps</span></strong> · بر اساس ' +
    persianNumber(reviewCount) +
    " نظر · آخرین تغییر ثبت‌شده در Google: " +
    persianGregorianDate(observedAt) +
    ' — <a href="https://www.google.com/maps?cid=' +
    release.clinic.cid +
    '" rel="external noopener">مشاهده نظرها</a></div>';
