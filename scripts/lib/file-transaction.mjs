...
export async function commitTextFiles(
  entries,
  { transactionId = randomUUID(), beforeCommit } = {},
) {
  if (!Array.isArray(entries) || entries.length === 0)
    throw new Error("File transaction requires at least one entry");
  ...
  for (const entry of normalized) {
    if (seen.has(entry.file))
      throw new Error(`Duplicate transaction target: ${entry.file}`);
    seen.add(entry.file);
    if (!(await exists(entry.file)))
      throw new Error(`Transaction target does not exist: ${entry.file}`);
  }
  ...
  try {
    for (const record of records) {
      if ((await exists(record.staged)) || (await exists(record.backup)))
        throw new Error(`Transaction residue collision: ${record.file}`);
      await writeFile(record.staged, record.content, "utf8");
    }

    for (const [index, record] of records.entries()) {
      if (beforeCommit)
        await beforeCommit({ index, file: record.file, transactionId });
  ...
  return {
    committed: records.map((record) => record.file),
    transactionId,
    cleanupResidue,
  };
}
