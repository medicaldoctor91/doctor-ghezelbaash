...
const normalizeEntryName = (value) => {
  ...
  if (
    ...
    throw new Error(`Invalid ZIP entry name: ${name || "<empty>"}`);
...
export async function walkFiles(
  ...
  for (const entry of (await readdir(directory, { withFileTypes: true })).sort(
    ...
    if (entry.isDirectory())
      files.push(...(await walkFiles(absolute, { prefix: relative, filter })));
...
export function createDeterministicZip(entries) {
  ...
  if (entries.length > 0xffff)
    throw new Error(`ZIP entry ceiling exceeded: ${entries.length}`);
  ...
  for (const entry of normalized) {
    ...
    if (
      ...
      throw new Error(`ZIP32 size ceiling exceeded: ${entry.name}`);
  ...
  if (centralDirectory.length > 0xffffffff || offset > 0xffffffff)
    throw new Error("ZIP32 central-directory ceiling exceeded");
