...
const initialInvariantNeedles=['کرایولیپولیز','ارائه یا توصیه نمی‌شود','منسوخ','CC BY 4.0','اطلاعات حساس پزشکی'];
function deferDeepPresentation(fullContent,canonicalGraph){
  const questions=canonicalGraph['@graph'].filter(n=>hasType(n,'Question'));
  const answers=canonicalGraph['@graph'].filter(n=>hasType(n,'Answer'));
  const answerNeedles=answers.flatMap(a=>[a.description,a.text]).filter(x=>typeof x==='string').map(normalizeVisible).filter(Boolean);
  ...
  const shouldKeep=(node,raw,state)=>{const tag=node.tagName,classes=classSet(node);if(/^h[1-6]$/.test(tag)||['script','style','noscript'].includes(tag))return true;if([...classes].some(x=>criticalKeepClasses.has(x)))return true;if(/<(?:video|picture|img|track)\b/i.test(raw))return true;if(/\bid=["'](?:medical-content-governance|dr-saeed-ghezelbash-aesthetic-clinic-facts)["']/i.test(raw))return true;if(initialInvariantNeedles.some(needle=>raw.includes(needle)))return true;const visible=normalizeVisible(raw);if(visible&&answerNeedles.some(needle=>visible.includes(needle)))return true;if(tag==='p'&&state.keptParagraphs<1){state.keptParagraphs++;return true}if(['address','dl'].includes(tag))return true;return false};
  ...
  for(const node of all){if(!classSet(node).has('final-collapsible-content'))continue;const loc=node.sourceCodeLocation;if(!loc)continue;const raw=fullContent.slice(loc.startOffset,loc.endOffset),visible=normalizeVisible(raw);if(/<(?:video|picture|img|track)\b/i.test(raw)||answerNeedles.some(needle=>visible.includes(needle))||initialInvariantNeedles.some(needle=>raw.includes(needle)))continue;candidates.push({start:loc.startOffset,end:loc.endOffset,raw})}
  candidates.sort((a,b)=>a.start-b.start||(b.end-b.start)-(a.end-a.start));let selected=[];
  ...
  const probePlaceholder=item=>{const ids=[...new Set(idsIn(item.raw))],aliases=ids.map(id=>`<span id="${escapeAttr(id)}"></span>`).join('');return `<span aria-hidden="true" class="deferred-fragment-placeholder" data-deferred-fragment="probe">${aliases}</span>`};
  let probe=fullContent;for(const item of [...selected].sort((a,b)=>b.start-a.start))probe=probe.slice(0,item.start)+probePlaceholder(item)+probe.slice(item.end);
  const probeVisible=normalizeVisible(probe),missingAnswerIds=new Set(answers.filter(a=>typeof a.text==='string'&&normalizeVisible(a.text)&&!probeVisible.includes(normalizeVisible(a.text))).map(a=>a['@id']));
  if(missingAnswerIds.size){
    const protectedRanges=[];
    for(const question of questions){if(!missingAnswerIds.has(question.acceptedAnswer?.['@id']))continue;let fragment='';try{fragment=decodeURIComponent(new URL(question.url||question['@id']).hash.slice(1))}catch{}if(!fragment)continue;for(const chunk of chunks){const loc=chunk.sourceCodeLocation;if(!loc)continue;const raw=fullContent.slice(loc.startOffset,loc.endOffset);if(new RegExp(`\\bid=["']${escapeRegExp(fragment)}["']`,'i').test(raw)){protectedRanges.push({start:loc.startOffset,end:loc.endOffset});break}}
    }
    selected=selected.filter(item=>!protectedRanges.some(r=>item.start>=r.start&&item.end<=r.end));
  }
  ...
  return{presentation,css,cssName,entries:entries.length,deferredSourceBytes:entries.reduce((sum,x)=>sum+Buffer.byteLength(x.html),0),presentationBytes:Buffer.byteLength(presentation),protectedAnswerChunks:missingAnswerIds.size};
}
...
export async function assembleCanonicalContent({root=process.cwd(),graph}={}){
  ...
  return{content:projectionGenerator?fullContent:deferred.presentation,fullContent,presentationContent:deferred.presentation,deferredCss:deferred.css,deferredCssName:deferred.cssName,deferredEntryCount:deferred.entries,deferredSourceBytes:deferred.deferredSourceBytes,presentationBytes:deferred.presentationBytes,protectedAnswerChunks:deferred.protectedAnswerChunks,names,inserted,fullInserted,dedupedParagraphs:deduped.removed,dedupedParagraphBytes:deduped.removedBytes};
}
