...
export function parseCssRules(css) {
  ...
  const parse = (segment, conditions = []) => {
    ...
    while (cursor < segment.length) {
      ...
      const boundary = findBoundary(segment, cursor);
      if (!boundary) break;
      ...
      const close = closingBrace(segment, boundary.index),
        body = segment.slice(boundary.index + 1, close);
      if (prelude.startsWith("@")) {
        if (/^@(media|supports|layer|container)\b/i.test(prelude))
          parse(body, [...conditions, prelude]);
      } else {
...
export function isMaxWidthRule(rule, widthPx) {
  return rule.conditions.some((condition) =>
    compact(condition).includes(`max-width:${widthPx}px`),
  );
}

export function mediaRuleAppliesAtWidth(
  rule,
  widthPx,
  { rootFontPx = 16 } = {},
) {
  const media = rule.conditions.filter((condition) =>
    /^\s*@media\b/i.test(condition),
  );
  if (!media.length) return false;
  const toPx = (number, unit) =>
    Number(number) * (unit.toLowerCase() === "rem" ? rootFontPx : 1);
  let widthConstraintCount = 0;
  const applies = media.every((condition) => {
    const matches = [
      ...condition.matchAll(
        /\((min|max)-width\s*:\s*(\d+(?:\.\d+)?)(px|rem)\)/gi,
      ),
    ];
...
export function compactCssValue(value) {
