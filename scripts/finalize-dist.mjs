...
const rdfLock=JSON.parse(await readFile(path.join(generated.semantic,'rdf-lock.json'),'utf8'));
...
if(graphIntegrity.duplicateIds.length||graphIntegrity.danglingSameSiteCount)throw new Error(`Graph closure failed: duplicates=${graphIntegrity.duplicateIds.length}, dangling=${graphIntegrity.danglingSameSiteCount}`);
...
const [redirectsBytes,canonicalRedirectsBytes]=await Promise.all([
  readFile(path.join(dist,'_redirects')),
  readFile(path.join(root,'public/_redirects')),
]);
const redirectsSha256=shaHex(redirectsBytes);
const redirectsPreservedByteIdentical=redirectsBytes.equals(canonicalRedirectsBytes);
const manifest={
...
const headersBytes=await readFile(path.join(dist,'_headers')),manifestBytes=await readFile(manifestPath);
const attestation={schemaVersion:'1.0',baseRelease:release.release,liveRevision,generatedAt,sourceCommit:liveRevision,artifactManifestSha256:shaHex(manifestBytes),indexHtmlSha256:shaHex(Buffer.from(html)),liveObservationSha256:shaHex(liveBytes),currentReleaseMatrixSha256:shaHex(await readFile(currentMatrixPath)),headersSha256:shaHex(headersBytes),conceptDoi:release.dataset.zenodo.conceptDoi,versionDoi:release.dataset.zenodo.versionDoi,integrity:'CURRENT_SERVING'};
