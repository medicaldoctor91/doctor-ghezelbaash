...
const headIds=await readJson('src/data/semantic/head-ids.json'),personHeadProfile=headProfile.nodes?.[release.primaryEntity.id],clinicHeadProfile=headProfile.nodes?.[release.clinic.id],allowedHeadMemberships=new Set(personHeadProfile?.refAllow?.memberOf||[]);
