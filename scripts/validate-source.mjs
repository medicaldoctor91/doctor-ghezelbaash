...
if(!assembled.content.includes('id="clinical-aesthetic-decision-pathway"')||!assembled.content.includes('class="clinical-identity-list"'))fail('Physician-specific 2026 presentation/decision pathway surface missing');
...
if(!source.includes('class="clinical-identity-disclosure"')||!source.includes('Verified professional identity of Dr. Saeed Ghezelbash')||/<details[^>]*clinical-identity-disclosure[^>]*\sopen(?:\s|=|>)/i.test(source))fail('Clinical identity disclosure contract drift');
if(!/<button\b(?=[^>]*hero-search-launch)(?=[^>]*aria-label="باز کردن جست‌وجوی راهنمای جامع")[^>]*>/i.test(source)||/<button\b[^>]*hero-search-launch[^>]*>[\s\S]*?<kbd\b/i.test(source)||/<button\b[^>]*class="[^"]*\bhero-action\b[^"]*\bhero-search-launch\b/i.test(source))fail('Accessible compact Hero search launcher contract drift');
