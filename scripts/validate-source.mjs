...
const requiredFiles=[
  ...
  'src/data/semantic/head-profile.json','src/data/semantic/support-profile.json','src/data/semantic/shapes.ttl','src/data/evidence-registry.json','src/data/evidence-snapshot.json','src/data/volatile-facts.json','src/data/render-calibration.json',
  ...
  'scripts/update-render-calibration.mjs','scripts/generate-retrieval-projections.mjs','scripts/generate-descriptors.mjs','scripts/finalize-dist.mjs','scripts/promote-release.mjs','scripts/write-release-attestation.mjs','scripts/zenodo_release.py','scripts/validate-media-references.mjs','scripts/validate-release-contract.mjs','scripts/validate-semantic-html.mjs','scripts/platform-contract.mjs','scripts/huggingface.mjs'
];
for(const f of requiredFiles)await access(path.join(root,f));

const readSource=file=>readFile(path.join(root,file),'utf8');
const [pkg,baseGenerator,retrievalGenerator,descriptorGenerator,finalizer,mediaReferenceValidator,calibrationWriter,hfVerifier,baseLayout,pageAssetsCompiler,graphCompiler,semanticCompiler,retrievalCompiler,contactCompiler]=await Promise.all([
...
if(releaseSteps.indexOf('npm run release:attest')<releaseSteps.indexOf('npm run compile:dist')||releaseSteps.indexOf('npm run release:attest')>releaseSteps.indexOf('node scripts/package-dist.mjs'))fail('Release attestation ordering drift');
if(!hasStep('build','npm run prepare:generated')||!hasStep('build','npm run validate:source')||!hasStep('build','npm run compile:dist'))fail('Build source/compiler ordering drift');
for(const required of ['astro build','npm run indexnow:prepare','npm run descriptors:finalize','node scripts/finalize-dist.mjs'])if(!hasStep('compile:dist',required))fail(`DIST compiler missing ${required}`);
...
for(const [name,profile] of [['head',headProfile],['support',supportProfile]])for(const ref of profile.ids||[])if(!graphIds.has(ref))fail(`${name} profile references missing graph node ${ref}`);
...
const headIds=headProfile.ids,personHeadProfile=headProfile.nodes?.[release.primaryEntity.id],clinicHeadProfile=headProfile.nodes?.[release.clinic.id],allowedHeadMemberships=new Set(personHeadProfile?.refAllow?.memberOf||[]);
...
const primaryClinicName='کلینیک زیبایی دکتر سعید قزلباش',clinicNamePolicy=arr(clinicHeadProfile?.valueAllow?.name),clinicImageRefPolicy=clinicHeadProfile?.refAllow?.image;
if(clinicNamePolicy.length!==1||clinicNamePolicy[0]!==primaryClinicName||!Array.isArray(clinicImageRefPolicy)||clinicImageRefPolicy.length)fail('Google Organization/LocalBusiness projection must use one primary name and direct image URLs');
const canonicalClinicNames=arr(clinic.name).map(value=>value?.['@value']??value),canonicalClinicImages=arr(clinic.image);
if(!canonicalClinicNames.includes(primaryClinicName)||!canonicalClinicNames.includes('Dr. Saeed Ghezelbash Aesthetic Clinic')||canonicalClinicImages.filter(value=>typeof value==='string').length<6||!canonicalClinicImages.some(value=>id(value)===`${release.canonicalUrl}#image-doctor-ghezelbaash-clinic-logo`))fail('Canonical multilingual Clinic name/image richness drift');
...
if(!source.includes('id="saeed-ghezelbash"'))fail('Canonical physician H1 missing');
if(source.includes('Public Knowledge Graph')||source.includes('doctor-ghezelbaash-structured-data-repository'))fail('Visible content contains a machine Dataset landing surface');
...
if(!source.includes('google-maps-clinic-reputation-current'))fail('Current reputation slot missing');
...
const redirects=await readFile(path.join(root,'public/_redirects'),'utf8'),sources=new Set();
...
console.log(JSON.stringify({stage:'SOURCE_SEMANTIC_CONTRACT',release:release.release,services:registered.size,answers:answers.answers.length,renderChunks:renderChunkIds.length,canonicalWriterTopology:'PASS',projectionCompilerTopology:'PASS',googleForwardPageTopology:'PASS',googleOrganizationProjection:'PASS',platformContract:'PASS',authoritySurfaceContract:'PASS',coreWikidataOwnership:'PASS',headProjectionPolicy:'SOURCE_VALIDATED',generatedFilePrerequisite:'NONE',buildSourceMutation:'FORBIDDEN',descriptorWriterPhases:['source-input','dist-final'],integrity:'PASS'},null,2));
