...
await fs.writeFile('src/content/aeo-data/google-maps-rating.json', `${JSON.stringify(json, null, 2)}\n`, 'utf8');

await updateJsonFile('src/content/aeo-data/ghezelbaash-entity-dataset.json', (dataset) =>
  updateRatingInEntityDataset(dataset, json)
);

await updateJsonFile('src/content/graph-sources/entity-registry-source.json', (graphData) =>
  updateClinicRatingInGraph(graphData, json)
);
