...
const liveDoc={
  ...
  item:{
    ...
    '@type':'MedicalClinic',
    identifier:[{propertyID:'Google Place ID',value:placeId}],
    additionalProperty:[
...
const baselineEvidence=[...new Set(evidenceEntries
  .filter(e=>e.tier==='A'&&arr(e.supports).some(s=>stableSupportKinds.has(s)))
  .map(e=>e.id))];
...
const evidenceRefsFor=node=>{
  ...
  walk(node);
  return [...new Set(found.map(x=>evidenceById.has(x)?x:evidenceByUrl.get(x)).filter(Boolean))];
};
const relatedEvidenceFor=text=>{
  const tokens=String(text||'').toLowerCase().split(/[^\p{L}\p{N}]+/u).filter(t=>t.length>=5&&!['procedure','service','medical','treatment','assessment','aesthetic','doctor','physician','kermanshah'].includes(t));
  if(!tokens.length)return [];
  return evidenceEntries.filter(e=>{
    const hay=`${e.id||''} ${e.url||''} ${arr(e.supports).join(' ')}`.toLowerCase();
    return tokens.some(t=>hay.includes(t));
  }).map(e=>e.id);
};
const answerEvidence=new Map();
for(const row of answerRegistry.answers){
  const q=byId.get(row.questionId),a=byId.get(row.answerId),source=byId.get(row.sourceUrl);
  answerEvidence.set(row.answerId,[...new Set([...baselineEvidence,...evidenceRefsFor(q),...evidenceRefsFor(a),...evidenceRefsFor(source),...relatedEvidenceFor(row.questionId)])]);
}
...
const commonRow=(language,scope,answer,stable)=>({
  language,
  query_scope:scope,
  practice_location:'Kermanshah, Iran',
  preferred_entity:release.primaryEntity.wikidata,
  preferred_entity_iri:release.primaryEntity.id,
  clinic_entity:release.dataset.supportingClinicWikidata,
  dataset_entity:release.dataset.wikidata,
  dataset_iri:release.dataset.id,
  release:release.release,
  version_doi:release.dataset.zenodo.versionDoi,
  retrieval_priority:policy.retrievalPriority,
  positioning_mode:policy.positioningMode,
  answer_id:answer.answerId,
  stable_evidence_refs:[...new Set(stable)],
...
const detectLanguage=text=>{
  const s=String(text||'');
  if(/[ۆێڕڵڤژ]/u.test(s))return 'ckb';
  if(/[أإآةى]/u.test(s)||/(^|\s)(في|أفضل|طبيب|علاج|عيادة)(\s|$)/u.test(s))return 'ar';
...
const serviceAnswerFor=service=>{
  const slug=service.id.split('#').pop()?.toLowerCase()||'';
  const tokens=slug.split(/[^a-z0-9]+/).filter(t=>t.length>=5&&!['procedure','service','medical','treatment','assessment','aesthetic'].includes(t)).sort((a,b)=>b.length-a.length);
  return answerRegistry.answers.find(r=>tokens.some(t=>r.questionId.toLowerCase().includes(t)))||intentAnswer['aesthetic-physician'];
};
...
for(const service of publishableServices){
  ...
  for(const alias of aliases){
    ...
    if(exactScope==='unspecified'){
      for(const scope of ['Kermanshah','Iran'])variants.push({query:`${alias}${scopeSuffix[lang][scope]}`.trim(),scope});
    }
...
console.log(JSON.stringify({
  ...
  rowsWithStableEvidence:dedup.filter(x=>x.stable_evidence_refs.length).length,
