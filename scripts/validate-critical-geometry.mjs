...
const requiredCriticalTokens=[
  'padding:1rem .85rem calc(7.2rem + env(safe-area-inset-bottom));padding-inline:.78rem',
  '.hero-action--search{appearance:none}',
  '.verified-identity-core{margin:.85rem 0;padding:.65rem .8rem',
  '.verified-identity-core code{direction:ltr;unicode-bidi:isolate;font-size:.9em}',
  '.quick-actions__top{width:2.15rem;height:2.15rem}',
  '.quick-actions__top::before{inset:-.35rem}',
  '.quick-actions__item--search{appearance:none;border:0;background:transparent;font:inherit',
...
const deferredChecks=[
  [/\.hero-action--search\{[^}]*appearance:none/s,'deferred Hero search button appearance'],
  [/\.verified-identity-core\{[^}]*margin:0?\.85rem 0;[^}]*padding:0?\.65rem 0?\.8rem;[^}]*font-size:0?\.94rem;[^}]*line-height:1\.8/s,'deferred verified identity geometry'],
  [/\.verified-identity-core code\{[^}]*direction:ltr;[^}]*unicode-bidi:isolate;[^}]*font-size:0?\.9em/s,'deferred verified identity code geometry'],
  ...
  [/\.quick-actions__item--search\{[^}]*appearance:none;[^}]*border:0;[^}]*background:transparent;[^}]*font:inherit/s,'deferred dock search-button reset'],
...
const convergenceStart=criticalMobileCss.indexOf('padding-inline:.78rem');
...
console.log('Critical geometry convergence validated: mobile main, Hero identity, search controls, and floating dock critical geometry match deferred final values.');
