...
register('AC-03', 'تصویر اصلی، اعتبار و دو CTA', [[personBlock.includes('data-lcp-portrait') && personBlock.includes('loading="eager"') && personBlock.includes('fetchpriority="high"'), 'LCP contract incomplete'], [(personBlock.match(/data-primary-cta=/gu) ?? []).length === 2, 'primary CTA count mismatch'], [personBlock.includes('رزرو وقت مشاوره رایگان') && personBlock.includes('گفت‌وگوی آنلاین با دکتر قزلباش'), 'CTA labels missing'], [personBlock.includes('۱۶۳ ارزیابی Google Maps'), 'rating provenance missing']], { primaryCtas: (personBlock.match(/data-primary-cta=/gu) ?? []).length });
...
register('AC-08', 'تفکیک offered / evaluated / referral-context', [['offered', 'evaluated', 'referral-context'].every((value) => coverageRelationships.has(value)), `relationships: ${[...coverageRelationships].join(', ')}`]], { coverageRelationships: [...coverageRelationships].sort() });
