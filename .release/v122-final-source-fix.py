...
# Permanent, field-specific release-date rollover tool. It preserves DOI/record/history identity,
# medical review truth and every historical release entry except the current publication date.
...
# Medical review truth is independent of publication/release date on every answer/Markdown projection.
p=ROOT/'scripts/generate-projections.mjs'; s=p.read_text()
s2=s.replace('REVIEWED_AT: ${release.dateModified}','REVIEWED_AT: ${release.medicalReviewedAt}')
s2=s2.replace('# Release ${release.release}; reviewed ${release.dateModified}; provenance-rich canonical answer records','# Release ${release.release}; reviewed ${release.medicalReviewedAt}; provenance-rich canonical answer records')
s2=s2.replace('> Release: ${release.release} | Reviewed: ${release.dateModified}','> Release: ${release.release} | Reviewed: ${release.medicalReviewedAt}')
...
# finalize-dist must write correct truth on first pass; no synthetic 05:58 timestamp and no legacy Dataset names.
...
# Remove obsolete count-as-ceiling fields that are no longer read by runtime code; retain real budgets and release-derived RDF lock.
...
for key in obsolete:
    pat=re.compile(rf'\b(?:inv|invariants)\s*\.\s*{re.escape(key)}\b')
    refs=[]
    ...
    inv.pop(key,None)
inv['contractClasses']={
  'architectural':{'singleCanonicalPage':True,'maxHtmlBytesField':'maxHtmlBytes','googlebotFetchBudgetBytesField':'googlebotFetchBudgetBytes','maxOrphanGraphNodesField':'maxOrphanGraphNodes','maxHeadGraphBytesField':'maxHeadGraphBytes','maxSupportGraphBytesField':'maxSupportGraphBytes'},
  'strategic':{'aggressiveHeadingSet':'src/data/visible-contract.json#protected.aggressiveHeadings','instagramHeadingAssociationSet':'src/data/visible-contract.json#protected.instagramHeadingLinks','serviceSet':'src/data/service-registry.json#services[publishable=true]','physicianFirstIdentity':True},
  'releaseDerivedMeasurements':{'rdfTripleCountField':'externalRdfTripleCount','renderChunkSet':'src/data/render-calibration.json','graphNodeCount':'measured-at-build','serviceCount':'derived-from-service-registry','answerCount':'derived-from-answer-registry','supportNodeCount':'derived-from-projection','htmlBytes':'measured-at-build'}
}
inv_path.write_text(json.dumps(inv,ensure_ascii=False,indent=2)+'\n')

# Strengthen source contract: no target-filling HTML and explicit invariant classes.
...
# Strengthen final DIST contract for medical review independence and descriptor identity/date semantics.
...
print(json.dumps({'materialized':True,'scope':'release-date-rollover-medical-review-descriptor-invariant-truth'},indent=2))
