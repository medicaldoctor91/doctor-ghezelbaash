export const researchProfile = {
  bibliographyUrl: 'https://www.ncbi.nlm.nih.gov/myncbi/saeed.ghezelbash.1/bibliography/public/',
  orcid: 'https://orcid.org/0009-0001-9346-8475',
  publications: [
    {
      key: 'healthcare-2021-1169',
      title: 'Individuals with Major Depressive Disorder Report High Scores of Insecure-Avoidant and Insecure-Anxious Attachment Styles, Dissociative Identity Symptoms, and Adult Traumatic Events',
      datePublished: '2021-09-06',
      doi: '10.3390/healthcare9091169',
      pubmed: 'https://pubmed.ncbi.nlm.nih.gov/34574943/',
      pmid: '34574943',
      pmcid: 'PMC8469763',
      authorNameVariant: 'Mohammad Saeed Ghezelbash',
      entityUse: 'research-identity-disambiguation'
    },
    {
      key: 'ijpm-2016-77',
      title: 'Effects of Omega-3 Supplement in the Treatment of Patients with Bipolar I Disorder',
      datePublished: '2016',
      doi: '10.4103/2008-7802.182734',
      pubmed: 'https://pubmed.ncbi.nlm.nih.gov/27280013/',
      pmid: '27280013',
      pmcid: 'PMC4882968',
      authorNameVariant: 'Ghezelbash MS',
      entityUse: 'research-identity-disambiguation'
    }
  ]
};
