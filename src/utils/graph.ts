...
const imageNodes = (assets: AssetManifest): JsonLdNode[] =>
    ...
    return [
      {
        ...
        width: asset.width,
        height: asset.height,
        ...(asset.alt && !asset.alt.includes('TODO_VISIBLE_CONTENT')
          ? { caption: asset.alt, name: asset.alt }
          : {}),
      },
    ];
  });
...
const faqNodes = (): JsonLdNode[] => {
  ...
  const questionNodes: JsonLdNode[] = faqItems.flatMap((item) => [
    {
      '@id': `${SITE_URL}#${item.id}`,
      '@type': 'Question',
      name: item.question,
...
const topicNodes = (): JsonLdNode[] => {
  ...
  return [
    {
      '@id': `${SITE_URL}#aesthetic-defined-terms`,
      '@type': 'DefinedTermSet',
      name: 'واژگان پزشکی زیبایی',
      hasDefinedTerm: terms.map((term: { id: string }) => asId(`${SITE_URL}#term-${term.id}`)),
    },
    ...terms.flatMap((term: { id: string; name: string; description?: string }) => [
      {
        '@id': `${SITE_URL}#term-${term.id}`,
        '@type': 'DefinedTerm',
        name: term.name,
        ...(term.description ? { description: term.description } : {}),
        inDefinedTermSet: asId(`${SITE_URL}#aesthetic-defined-terms`),
      },
      {
        '@id': `${SITE_URL}#procedure-${term.id}`,
...
const videoNodes = (assets: AssetManifest): JsonLdNode[] =>
  videos.flatMap((video) => {
    ...
    if (
      ...
      !asset.poster ||
      !asset.captions ||
      video.title.includes('TODO_VISIBLE_CONTENT') ||
      video.description.includes('TODO_VISIBLE_CONTENT')
    ) {
      return [];
    }
    return [
...
export const buildCanonicalGraph = (assets: AssetManifest): CanonicalGraph => {
  const nodes: JsonLdNode[] = [
    ...
    },
    {
      ...
      hasPart: sections.map((section) => asId(`${SITE_URL}#section-${section.id}`)),
    },
    {
      '@id': doctor.id,
      '@type': 'Person',
      ...
      mainEntityOfPage: asId(pageId),
      ...(assets.images['doctor-portrait']
        ? { image: asId(`${SITE_URL}#image-doctor-portrait`) }
        : {}),
    },
    {
      '@id': clinic.id,
      ...
      ...(assets.images['clinic-interior']
        ? { image: asId(`${SITE_URL}#image-clinic-interior`) }
        : {}),
    },
    {
    ...
    },
    {
      ...
      itemListElement: sections.map((section, index) => ({
        '@type': 'ListItem',
        position: index + 1,
        item: asId(`${SITE_URL}#section-${section.id}`),
      })),
    },
    {
      '@id': `${SITE_URL}#dataset-canonical-graph`,
      '@type': 'Dataset',
      name: 'Canonical JSON-LD knowledge graph for ghezelbaash.ir',
      url: `${SITE_URL}graph.jsonld`,
      inLanguage: ['fa-IR', 'en'],
      distribution: {
        '@type': 'DataDownload',
        contentUrl: `${SITE_URL}graph.jsonld`,
        encodingFormat: 'application/ld+json',
      },
      isBasedOn: asId(pageId),
    },
    {
      '@id': `${SITE_URL}#dataset-retrieval-corpus`,
      '@type': 'Dataset',
      ...
      distribution: [
        ...
        {
          '@type': 'DataDownload',
          contentUrl: `${SITE_URL}assets/data/chunks.json`,
    ...
    ...sections.map(
      (section): JsonLdNode => ({
        '@id': `${SITE_URL}#section-${section.id}`,
        '@type': 'WebPageElement',
        name: section.title,
