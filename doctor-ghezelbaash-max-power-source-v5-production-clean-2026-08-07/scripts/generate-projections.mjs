...
const volatileFacts=JSON.parse(await readFile(path.join(data,'volatile-facts.json'),'utf8'));
...
try{
  const names=(await readdir(sourceDir)).filter(x=>/\.(?:md|html)$/.test(x)).sort();
  if(names.length){
    const assembled=(await Promise.all(names.map(n=>readFile(path.join(sourceDir,n),'utf8')))).join('');
    await writeFile(path.join(root,'src/content/home.md'),assembled);
  }
}catch{}
...
for(const q of graph['@graph'].filter(n=>types(n).includes('Question'))){
  ...
  answerRecords.push({q,a,sourceUrl,evidenceIds,claimEvidenceIds,entityEvidenceIds,sourceHash});
}
const answers=answerRecords.map(({q,a,sourceUrl,evidenceIds,claimEvidenceIds,entityEvidenceIds,sourceHash})=>`QUESTION_ID: ${q['@id']}
...
for(const {q,a,sourceUrl,evidenceIds,claimEvidenceIds,entityEvidenceIds,sourceHash} of answerRecords){
  provGraph.push({
    ...
    additionalProperty:[{'@type':'PropertyValue',propertyID:'Entity evidence IDs',value:entityEvidenceIds.join(' | ')}],
    dateModified:release.dateModified
  });
}
...
sitemap+='  </url>\n';
const machineSitemapResources=['graph.jsonld','graph.ttl','entity-facts.csv','answers.txt','knowledge.xml','llms.txt','index.md','llms-full.txt','datapackage.json','linkset.json','void.ttl','dcat.ttl','croissant.json','provenance.jsonld','evidence-snapshot.json','shapes.ttl','artifact-manifest.json'];
for(const rel of machineSitemapResources) sitemap+=`  <url><loc>${release.canonicalUrl}${rel}</loc><lastmod>${release.dateModified}</lastmod></url>\n`;
sitemap+='</urlset>\n';
