...
if(ldScripts.length!==2||execScripts.length!==1||!/id=["']site-runtime["']/i.test(execScripts[0].attrs)||styleBlocks.length<1||styles404.length<1)throw new Error(`Unexpected inline assets: styles=${styleBlocks.length}, ld=${ldScripts.length}, exec=${execScripts.length}`);
...
const mainCsp=`default-src 'none'; base-uri 'self'; script-src ${scriptHashes}; style-src ${styleHashes}; img-src 'self' data:; media-src 'self'; font-src 'self'; manifest-src 'self'; connect-src 'none'; object-src 'none'; frame-src 'none'; frame-ancestors 'none'; form-action 'self'; upgrade-insecure-requests`;
const csp404=`default-src 'none'; base-uri 'self'; style-src ${styles404.map(x=>`'sha256-${shaB64(Buffer.from(x))}'`).join(' ')}; img-src 'self' data:; font-src 'self'; frame-ancestors 'none'; form-action 'self'; object-src 'none'; upgrade-insecure-requests`;
...
for(const file of machine)headers=headers.replace(`{{DIGEST:${file}}}`,shaB64(await readFile(path.join(dist,file))));
...
const ids=[...html.matchAll(/\bid=["']([^"']+)["']/gi)].map(m=>m[1]),frags=[...html.matchAll(/\bhref=["']#([^"']+)["']/gi)].map(m=>m[1]),idSet=new Set(ids),missing=[...new Set(frags.filter(x=>!idSet.has(x)))].sort();
const files={};for(const f of await walk(dist)){if(f.rel==='artifact-manifest.json')continue;const b=await readFile(f.abs);files[f.rel]={bytes:b.length,sha256:shaHex(b)};}
